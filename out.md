...
Actualmente, Carlos comparte su experiencia en el campo del desarrollo
...
Por ultimo, te guiaré a través de los pasos para desplegar tus aplicaciones de React
...
A pesar de su popularidad, React no estuvo exento de controversia. En 2017,
...
React es una gran herramienta para construir interfaces de usuario. Su enfoque en
...
Vite es una herramienta que te permite crear un proyecto de JavaScript (Vanilla,
...
Y los componentes de tipo funcién, cuyos sinénimos serian: Stateless Components,
...
+ Diferenciacién (Diffing): React compara el nuevo V-DOM con el antiguo
...
Diferencias entre una aplicacién con y sin Context API
Creando un contexto
...
Ahora, en cualquier parte de nuestra aplicacién, podemos usar el hook useTheme
...
En este ejemplo, el reducer todosReducer maneja la acci6n ADD_TODO afiadiendo
...
2. Luego, se despacha la accién addTodo('Aprender React' ),queafiade una
nueva tarea al estado, El nuevo estado se imprimeyes:[ { id: 0, text:
'Aprender React' } ].

3. Después, se despacha la accién addTodo('Learn React’ ), que ajiade otra
tarea al estado. El nuevo estado se imprime yes:[ { id: 0, text:
‘Aprender React' }, { id: 1, text: 'Aprender Redux' } ].

Asi, puedes ver como el estado de la aplicacién cambia a medida que se despachan
las acciones.

Como puedes ver, Redux proporciona una forma estructurada y predecible de
manejar el estado global de una aplicacién. Sin embargo, también puede ser mas
complejo y verboso que otras soluciones como la Context API.
En el siguiente apartado, exploraremos cémo integrar Redux en una aplicacién
React y cémo puede ayudarnos a manejar el estado de una manera més eficiente y
escalable. También veremos la version de Redux Toolkit que hace mas sencillo poder
...
Vamos a crear una aplicacién de carrito de compras simple utilizando React y
Redux.

Creacién del proyecto
...
Instalacién de dependencias

Necesitaremos Redux para la gestién del estado y @reduxjs/toolkit para
simplificar la configuracién de Redux. Instala estas dependencias con el siguiente
comando:

1 $ npm install redux @reduxjs/toolkit react-redux

Creaci6n de archivos

Vamos a crear una estructura de archivos basica para nuestra aplicacién. Dentro del
...
Un slice incluye el reducer y las acciones, lo que simplifica la configuracién de
Redux. Definimos dos acciones, addToCart y removeFromCart, que afiaden y

eliminan articulos del carrito, respectivamente.

Acontinuaci6n, vamos a configurar el store de Redux. En sre/store. js, agregael
siguiente cédigo:
...
Cuando el usuario hace clic en el botén “Add to cart”, despachamos la accién
addToCart con el articulo. Cuando el usuario hace clic en el botén “Afiadir al

carrito”, despachamos la accién removeFromCart con el articulo.

Finalmente, vamos a utilizar nuestro componente de carrito de compras en nuestro
componente principal de la aplicacién. En src/App.js, agrega el siguiente
cédigo:
...
proporcionar el store de Redux a nuestra aplicacién. Luego, utilizamos nuestro
componente Cart.

Ejecucién y prueba de la aplicacion
Ahora, puedes ejecutar tu aplicacién con el siguiente comando:

1 $ npm run dev

Abre tu navegador y ve a http: //localhost: 5000 (0 el puerto donde se esté
ejecutando). Deberias ver tu aplicacién de carrito de compras en funcionamiento.

Este es un ejemplo basico de cémo puedes utilizar Redux en una aplicacién React.
A medida que tu aplicacién crece, puedes aiiadir mas slices a tu store para manejar
diferentes partes del estado de tu aplicacién.
Comparaci6n de soluciones de manejo de estado

En el mundo de React, existen varias soluciones para manejar el estado global
de una aplicacién. En este capitulo, hemos explorado en detalle dos de las
mas populares: Context API y Redux. Sin embargo, existen otras alternativas que
también son dignas de consideracién.

Acontinuacién, haremos una breve comparacién de estas soluciones.
Context API

La Context API es una solucién incorporada en React que permite compartir estado
...
para aplicaciones JavaScript. Es muy poderoso y flexible, y tiene una gran
...
API puede ser un poco complicada para los principiantes. Su versién Redux Toolkit

mejora esto y lo hace mas amigable.

Mobx

MobX es una libreria de gestién de estado que se centra en la programacion reactiva
...
arquitectura, permitiendo manejar el estado de la aplicacién fuera de cualquier
...
jotai toma un enfoque atémico para la gestién del estado global de React.
...
necesidad de memoizacién. Jotai escala desde un simple reemplazo de useState hasta
una aplicacién TypeScript empresarial con requisitos complejos.

Zustand
Zustand es una solucién de gestién de estado pequefia, répida y escalable. Zustand

tiene una API cémoda basada en hooks. No es boilerplate y no es “opinionado”, pero
tiene suficiente convencién para ser explicito y parecido a Flux.
...
La eleccién de la solucién de manejo de estado depende en gran medida de las ge uso, En tiltima instancia, la mejor solucién es la que mejor se adapta a tus
...
+ Facebook. (2023). Passing Data Deeply with Context. https://react.dev/learn/
...
En este capitulo, nos centraremos en un aspecto crucial de cualquier aplicaci6n web
moderna: La navegacién entre diferentes pantallas, vistas o paginas dentro de un
mismo sitio web.
...
“aplicacion de pagina tinica” (SPA o Single Page Application) y una aplicacién de
“renderizado del lado del servidor” (SSR 0 Server-Side Rendering).

SPA vs SSR

Una SPA, 0 Single Page Application, es una aplicacién web que carga una sola
pagina HTML y luego actualiza dinamicamente esa pagina a medida que el usuario
interacttia con la aplicacién. En lugar de enviar una nueva solicitud al servidor cada
...
rapidas y proporcionar una experiencia de usuario mas fluida. También se conoce
como client-side rendering (CSR 0 renderizado del lado del cliente).

Por otro lado, una aplicacién de renderizado del lado del Ssrvidor (SSR), como
...
usuario menos fluida en comparacién con una SPA.
...
es la unica opcién para el enrutamiento en React. Al final de este capitulo, también
...
aplicacién. Para hacerlo, necesitards importar los componentes que proporciona
React Router. Los més comunes son BrowserRouter, Routes, Route y Link.

Aqui tienes un ejemplo de cémo podrias configurar React Router en tu aplicacién:
...
nuestra aplicacién y la dota de la funcionalidad de enrutamiento. Link es un
componente que nos permite crear enlaces a diferentes rutas en nuestra aplicacién,
...
definir rutas en nuestra aplicacién. Como vimos en el ejemplo anterior, utilizamos
...
useHistory nos da acceso al historial de navegacién, que podemos usar para
...
al historial de navegacién. Esto significa que si el usuario presiona el botén
de retroceso en su navegador, volver a la pagina del formulario. Si en cambio
quieres reemplazar la entrada actual en el historial de navegacién (es decir, si
...
El cambio principal en la versién 6 de React Router es que ahora se basa en
componentes y hooks, en lugar de utilizar la configuracién de rutas como un objeto.
...
una mejor integracién con las caracteristicas nativas del navegador.

Aqui estd el ejemplo anterior actualizado a la versién 6 de React Router:
...
proporcionar el router a nuestra aplicacién. Dentro de RouterProvider,
...
6  // La funcién ‘toader* se utiliza para cargar datos basados
e\
...
10 // ~getUser> es una funcién hipotética que carga los
datos \
...
Hasta aqui llega nuestra exploracién de React Router. En el siguiente apartado,
...
dependiendo de las necesidades especificas de tu proyecto. A continuacién, te
...
React Navigation es una solucién de enrutamiento para aplicaciones méviles
...
lo que significa que si sabes como escribir aplicaciones usando JavaScript, puedes
...
Next Router es la solucién de enrutamiento integrada en Next.js, un framework
...
automaticamente disponible como una ruta. También soporta rutas dindmicas, lo
que te permite agregar parametros personalizados a tus URL.

Cada una de estas bibliotecas tiene sus propias ventajas y desventajas, y la eleccién
entre ellas dependera de las necesidades especificas de tu proyecto. Te recomiendo
que explores cada una de ellas para determinar cual es la mejor opcién para ti.
Conclusién
...
instalacién y configuracién de React Router, la creacién de rutas estaticas y

dinamicas, y la navegacién programatica.
...
aplicacién React, el objetivo es el mismo: proporcionar una experiencia de usuario

fluida y eficiente.

Esperamos que este capitulo te haya proporcionado una comprensién sdlida del
enrutamiento en React y cémo puedes utilizar React Router para manejar la
navegacién en tus aplicaciones. Con estos conocimientos, estas bien equipado para
...
Aprenderemos sobre las diferentes formas de ajiadir estilos a tu aplicacién React,
...
Building Web Apps with JavaScript. O'Reilly Media.
...
+ Expo, Software Mansion, y Callstack. (2021). React Navigation: Routing
...
siendo una biblioteca de JavaScript, ofrece varias formas de aplicar estilos CSS
a los componentes. Aunque JavaScript y CSS son tecnologias distintas, React ha
encontrado formas de combinarlas de manera efectiva para proporcionar una

experiencia de desarrollo mas fluida.
Formas de usar CSS en React

En el desarrollo web tradicional, los estilos CSS se aplican a los elementos HTML a
...
componentes utilizando un objeto JavaScript. Los nombres de las propiedades
...
2. Hojas de estilo CSS externas: Al igual que en el desarrollo web tradicional,
...
3. CSS Modules: Los médulos CSS son una extensién de la idea de hojas de estilo CSS.
externas. Permiten que los estilos se apliquen solo al componente que los importa,
evitando asi el problema de la colisién de nombres de clases.
...
permiten definir componentes con estilos adjuntos utilizando JavaScript. Este
...
Cada uno de estos métodos tiene sus ventajas y desventajas, y la eleccién
entre ellos dependera de tus necesidades especificas. En los siguientes apartados,
profundizaremos en cada uno de estos métodos, y también exploraremos cé6mo
...
componente, evitando asi los problemas de colisién de nombres de clases y la
propagacién de estilos no deseados. Con CSS Modules, cada archivo de estilos se
...
extensién . module. css. Por ejemplo, si tienes un componente llamado Button,
...
utilizando la sintaxis de importacién de ES6. El archivo de estilos se importara
...
componente como si fueran un objeto JavaScript. Los nombres de las clases se
...
no afectaran a otros componentes. Esto evita los problemas de colisién de
nombres de clases y la propagacién de estilos no deseados.
...
2. No es parte del estandar de CSS: CSS Modules es una extensién de CSS y no
es parte de su estandar. Esto significa que no todos los entornos de desarrollo
...
Como resumen, CSS Modules es una gran opcién si quieres encapsular los estilos a
nivel de componente y evitar los problemas de colisién de nombres de clases. Sin
embargo, si necesitas estilos dindmicos basados en las props de los componentes,
es posible que desees considerar una solucién de “CSS en JS” como styled-
components.
CSS en JS: Styled-components y otras soluciones

“CSS en JS” es una técnica que permite escribir CSS directamente dentro de
JavaScript. Esta técnica ha ganado popularidad en la comunidad de React debido
...
2. Crear un componente estilizado: Utiliza la funcién styled de styled-
components para crear un componente estilizado. Puedes escribir CSS normal
dentro de un Template Literal de JavaScript.
...
1 // Crear un componente de botén
...
En este ejemplo, el botén cambiara su estilo si se le pasa la prop primary. Si
primary es true, el botén tendra un fondo blanco y el texto sera de color
palevioletred.
...
En este ejemplo, TomatoButton es una extensién de Button, pero con un color y
un borde de color tomato.

Estos son solo dos ejemplos de lo que puedes hacer con Styled Components. Te

recomiendo que consultes la documentacién oficial para explorar mas a fondo sus
posibilidades.

Ventajas de styled-components

1. Estilos dinamicos: Con sty1ed- components, puedes definir estilos dinamicos
basados en las props de los componentes. Resumiendo todo, styled- components es una buena opcién si necesitas estilos
...
2 background-color: ${props => props.primary ? ‘blue’ : prefieras una solucién como CSS Modules.
...
3.Eliminacién automatica de CSS no utilizado: Styled-components elimina
automaticamente el CSS que no se utiliza, lo que puede ayudar a reducir el
tamaijio de tu bundle de JavaScript.
...
estilos se definen dentro de JavaScript, lo que puede ser confuso para los
...
vida de los componentes para cambiar el disefio de tus componentes en funcién
...
que puedes usar para acelerar el desarrollo de tus aplicaciones. Estas bibliotecas
proporcionan una amplia gama de componentes, desde botones y formularios
hasta barras de navegacién y modales, todos con estilos y funcionalidades ya
...
Ademas, necesitas envolver tu aplicacién con el ChakraProvider en tu archivo
...
Ahora, puedes empezar a usar los componentes de Chakra UI en tu aplicacién. Aqui
tienes un ejemplo de cémo puedes usar un botén y una caja de Chakra UI:
...
construir tus disefios. Button es un componente de botén que puedes personalizar

con la prop colorScheme.

Chakra UI también proporciona una amplia coleccién de otros componentes, como
...
+ React Bootstrap: Una versién de Bootstrap especificamente disefiada para
React.
...
componentes, desde controles de entrada y navegacién hasta visualizaciones y
utilidades.
...
disefio en React. Hemos comenzado con una introduccién a las diferentes formas
...
getting-started
Capitulo 7: Consumo de APIs y

comunicacion con el Servidor
Introducci6n

En una aplicacién web moderna, la comunicacién con servidores es también una
parte esencial. Ya sea para obtener datos, enviar datos, autenticar usuarios, 0
realizar cualquier otra operacién, las aplicaciones web necesitan interactuar con
...
interactuar con los servidores. Sin embargo, gracias a la flexibilidad de JavaScript y
...
servidores desde una aplicacién React:

+ Usando la API Fetch incorporada de forma nativa en JavaScript.
...
eleccién de uno sobre otro puede depender de las necesidades especificas de tu
proyecto. También discutiremos como manejar errores y el estado de carga, y como

implementar un sistema de autenticacion y autorizaci6n.

Antes de continuar, es importante tener un buen entendimiento de JavaScript
y de cémo funciona la comunicacién HTTP y la asincronia. Si necesitas repasar
...
En este ejemplo, fetch hace una peticién GET a la URL proporcionada. Cuando
...
Axios
Axios es una biblioteca de JavaScript que se puede usar para hacer peticiones

HTTP. A diferencia de Fetch, Axios tiene una API mas rica y es compatible
con navegadores mas antiguos. También tiene algunas caracteristicas adicionales,
...
En este ejemplo, axios.get hace una peticién GET a la URL proporcionada.
...
varios formatos, incluyendo JSON, texto plano, y FormData. La eleccién entre
...
Hook useEf fect para realizar la peticién HTTP cuando el componente se monta.
...
isLoading sera true mientras la peticién esté en curso, error contendra
cualquier error que ocurra durante la peticién, y data contendra los datos
recibidos.

Ademis, este Hook realizara peticiones de tipo GET. Para ello el Hook recibe como
argumento ur1, que es la URL a la que se hard la peticién.
...
cémo puedes implementarlo en tu aplicacién para obtener datos de una API:
...
necesitamos y realizar la peticién GET cuando el componente se monta.
Finalmente, mostramos los datos, un mensaje de carga, o un mensaje de error en el
renderizado del componente, dependiendo del estado de la peticién.

Este Custom Hook es una herramienta muy util que puedes reutilizar en diferentes
partes de tu aplicacién para manejar las peticiones HTTP de una manera mas
eficiente y ordenada. Y lo mejor es que lo puedes reutilizar en cualquier parte de tu
cédigo dénde hagas peticiones de este tipo.
...
podemos modificar el Hook useHttp para que devuelva una funcién que pueda ser
...
En este caso, el Hook useHttp devuelve una funcién sendRequest que puede
ser llamada para realizar una peticién HTTP. Esta funcién es envuelta en
...
En este ejemplo, usamos el Hook useHt tp para obtener los estados y la funcién
...
biblioteca que tiene una gran aceptacién, se trata de TanStack Query (0 React Query)
React Query (TanStack Query)

React Query, ahora conocida como TanStack Query, es una biblioteca que se describe
como la herramienta de obtencién de datos que faltaba para las aplicaciones
web. En términos mas técnicos, facilita la obtencién, almacenamiento en caché,
sincronizacion y actualizacién del estado del servidor en tus aplicaciones web.
...
Origen y evolucién a TanStack Query
...
sobre la obtencién de datos, o inventamos nuestras propias formas de obtener
...
componentes, o usar bibliotecas de gestién de estado de propdsito general para

almacenar y proporcionar datos asincronos en sus aplicaciones.

React Query surgié como una solucién a estos problemas, proporcionando una
...
34 <strong> {data.forks_count}</strong> + queryFn: Es una funcién que devuelve una promesa que eventualmente
...
Finalmente, en funcién del estado de la consulta, mostramos un mensaje de carga,
...
dos argumentos: Una funcién de mutacién y un objeto de opciones.
» Funcién de mutacin: Esta es una funcién que realiza la mutacion. En este
caso, estamos utilizando fetch para enviar un nuevo ToDo a nuestra API.
La funcién de mutacién puede ser cualquier funcién que devuelva una
promesa.

» Objeto de opciones: Este es un objeto que puede contener varias opciones
para configurar el comportamiento de la mutacién. En este caso, estamos
utilizando la opcién onSuccess para invalidar y volver a buscar las
consultas ‘todos’ cuando la mutacién se realiza con éxito.

+Realizando la mutacion: Para realizar la mutacién, lamamos al método
mutate en el objeto devuelto por useMutation. Pasamos los datos que
queremos enviar a nuestra API como argumento a mutate.

+ Manejando el estado de la mutacién: useMutation devuelve un objeto con
varios campos que representan el estado actual de la mutacion. En este caso,
estamos utilizando isLoading, error, y data.

isLoading: Es true mientras la mutacién esta en progreso y fal ‘se una vez
que se ha completado o ha fallado.

» error: Es null a menos que la mutacion haya fallado, en cuyo caso sera el
error que se produjo durante la mutacién.

» data: Son los datos obtenidos de la mutacion. Es undef ined hasta que la
mutacién se completa con éxito.

Finalmente, en funcién del estado de la mutacién, mostramos un mensaje de carga,
un mensaje de error, o los datos obtenidos de la API.
Manejo de errores y estado de carga

Vamos a dedicar un apartado a ver como se maneja el estado de carga y la gestion
de errores ya que son aspectos cruciales de cualquier aplicacién que interactiia con
una API.
...
Cuando realizamos una peticién a una API, siempre existe la posibilidad de que
...
En el caso de Fetch y Axios, estos lanzaran una excepcién si ocurre un error de red,
pero no si la peticién se realiza con éxito pero el servidor devuelve un cédigo de
...
ocurre un error durante la consulta o la mutacién, TanStack Query lo capturara
automaticamente y lo pondra a nuestra disposicién a través del campo error del
...
Ademas del manejo de errores, también es importante proporcionar feedback al
usuario sobre el estado de carga de la peticién. De nuevo, Fetch, Axios y TanStack
...
iniciar la peticién y actualizarla cuando la peticién se complete:
...
Estos son solo ejemplos basicos. En una aplicacién real, probablemente querrias
...
La autenticacién y la autorizacién son aspectos muy importantes de la seguridad
en las aplicaciones web.

La autenticacién se refiere al proceso de verificar la identidad de un usuario,
mientras que la autorizacién se refiere al proceso de verificar los permisos de un
usuario.

Implementaci6n de la autentica

En una aplicacién React, puedes implementar la autenticacién y la autorizacién
utilizando un API como backend. Aqui tienes un ejemplo basico de c6mo puedes
hacerlo:

1. Autenticacién: Cuando un usuario intenta iniciar sesién, envias sus
credenciales (como su nombre de usuario y contrasefia) a tu API. Si las
credenciales son correctas, tu API devuelve un token de autenticacién.
...
2. Almacenamiento del token: Almacenas el token de autenticacién en el
estado de tu aplicacién (0 en el almacenamiento local del navegador) para
...
3. Uso del token: Cuando realizas una peticién a tu API que requiere autenticacién,
incluyes el token de autenticacién en el encabezado Authorization de la
peticion.
...
4. Autorizacién: En tu API (Backend), verificas el token de autenticacién y
...
Aqui tienes un ejemplo de cémo puedes almacenar el token de autenticacién en
localStorage:
...
Podemos encapsular la légica de almacenamiento y recuperacién del token en
un Custom Hook para reutilizarla en diferentes partes de nuestra aplicacién. Aqui
...
se monta el componente. También proporcionamos una funcién saveToken que
...
Con este Hook, puedes manejar el token de autenticacién de manera eficiente
y reutilizar la légica de almacenamiento y recuperacién del token en diferentes

partes de tu aplicacién.

Soluciones de autenticacion y autorizacién de terceros

Si bien puedes implementar tu propio sistema de autenticacién y autorizacién,
...
- Auth: Auth es un servicio de autenticacién y autorizacién que proporciona
una API y una biblioteca de JavaScript que puedes utilizar para autenticar y
autorizar usuarios en tu aplicacién React.

-Firebase Auth: Firebase Auth es un servicio de autenticacién proporcionado
por Firebase, un producto de Google. Proporciona una API y una biblioteca de
JavaScript que puedes utilizar para autenticar usuarios con varios métodos,
incluyendo correo electrénico y contrasefia, Google, Facebook, GitHub y mas.
+NextAuth.js: NextAuth.js es una biblioteca de autenticacién para Next.js, un
...
autenticacién, incluyendo correo electrénico y contrasefia, Google, Facebook, y
mas.

Estas soluciones pueden ahorrarte mucho tiempo y esfuerzo, y también
proporcionan caracteristicas adicionales como la autenticacién en dos factores, la
recuperacién de la contraseiia, y la proteccién contra ataques de fuerza bruta. Sin
embargo, también pueden ser mAs costosas y menos flexibles que implementar tu
propio sistema de autenticacién y autorizacién.
Conclusién

En este capitulo, hemos explorado en profundidad cémo interactuar con APIs y
manejar la comunicacién con el servidor en una aplicacién React. Hemos aprendido
arealizar peticiones HTTP utilizando Fetch y Axios, y a manejar el estado de la carga
y los errores. También hemos explorado cémo crear y utilizar Custom Hooks para
...
obtencién de datos que puede simplificar y optimizar la comunicacién con
...
Finalmente, hemos discutido la autenticacién y la autorizacién, dos aspectos
...
En el préximo capitulo, nos centraremos en la optimizacién y el rendimiento.
Aprenderemos cémo mejorar la velocidad y la eficiencia de nuestra aplicacién
React.
Referencias
...
Capitulo 8: Optimizacién y Rendimiento
Introducci6n

Una de las razones por las que React ha ganado popularidad en el mundo
de la programacién web es su enfoque en la eficiencia y el rendimiento. Sin
...
rendimiento de nuestras aplicaciones React. Desde la virtualizacién y paginacién
de listas hasta la memoizacién de componentes, el code splitting y el lazy
loading, cada seccién de este capitulo se centrard en una técnica especifica,
...
joan Le6n (@nucliweb) y a Estela Franco (@guaca). Ambos son personas expertas en
el campo de la optimizacién del rendimiento web y ofrecen valiosos recursos para

profundizar en estos temas.
Paginacion de listas

La paginacién de listas es una técnica crucial en el desarrollo de aplicaciones web
...
aplicacién, la paginacién divide los datos en paginas mas manejables.

El usuario puede entonces navegar a través de estas paginas de manera
incremental. Esta técnica no sdlo mejora el rendimiento de la aplicacién al reducir
la carga inicial de datos, sino que también mejora la experiencia del usuario al
presentar los datos de manera ms organizada y manejable.

Paginacién con React-Router-Pagination

Un paquete muy ttil para implementar la paginacién en aplicaciones React
es react-router-pagination. Esta biblioteca proporciona una serie de
componentes y hooks para manejar la paginacién en combinacién con react-
router, el estandar de facto para la gestién de rutas en React.

Aqui te dejo un ejemplo de cémo se podria implementar la paginacién de una lista
...
enlaces de paginacién. Cada pagina muestra 10 elementos de la lista (definidos por
...
utiliza para mostrar los enlaces de paginacién en la parte inferior de la lista.

Este es un concepto basico de la paginacién en React con esta biblioteca. Te invito a

explorarla mas a fondo y practicar para dominar la paginacién de listas.
...
A diferencia del ejemplo anterior donde la paginacién era manejada
completamente en el cliente, React Query nos permite implementar una paginacién
del lado del servidor, lo que puede ser mucho mas eficiente para conjuntos de datos

muy grandes.

Aqui te presento un ejemplo de cémo implementar la paginacién de listas con React
Query:
...
33 data, funcién recibe la pagina actual como argumento.
...
39 queryFn: () => fetchProjects(page) , recientes después de una actualizacién. Una vez que los datos se vuelven
...
45 useEffect(() => { indicacién de si la consulta esta actualmente buscando datos (isFetching), y
...
Se usa la funcién set Page para cambiar la pagina actual cuando se presionan los
) botones de navegacién de la pagina.
}
...
La memoizacién es una técnica de optimizacién que se usa para acelerar las
aplicaciones al almacenar los resultados de operaciones costosas y reutilizindolos
en lugar de volver a ejecutar las operaciones. En React, la memoizacién puede
aplicarse a los componentes y las funciones para evitar el renderizado o los clculos

innecesarios.

En React, tenemos dos mecanismos principales para realizar la memoizacién:
React.memo yuseMemo.
...
useMemo es un hook que memoriza el resultado de una funcién. Es util cuando
...
En este ejemplo, useMemo toma dos argumentos: una funcién y una lista de
dependencias. La funcién se ejecutara y su resultado se memorizara la primera
...
useMemo devolvera el valor memorizado en lugar de volver a ejecutar la funcién.

La memoizacién puede ser una herramienta muy potente para optimizar tus
componentes React y prevenir renderizados innecesarios.

Sin embargo, como cualquier herramienta de optimizacién, debe usarse con
...
memoizacién mejora la velocidad de tu aplicacién.
Code Splitting y Lazy Loading

La optimizacién del rendimiento no siempre implica hacer que el cédigo existente
...
El code splitting es el proceso de dividir el cédigo de tu aplicacién en multiples
bundles (paquetes) que pueden ser cargados bajo demanda. Las herramientas
modernas como Vite y Next.js, que son cada vez mas populares para la creacién de
aplicaciones React, tienen soporte incorporado para ello.

Esto significa que cada vez que un usuario carga tu aplicacién, solo se esta
...
Para Vite, la configuracién de code splitting puede estar en el archivo
...
La aplicacién del code splitting puede tener un gran impacto en el rendimiento de
tu aplicacién, permitiéndote cargar solo el cédigo que es necesario, lo que mejora la
eficiencia y la experiencia del usuario.

Lazy Loading

El lazy loading es una técnica de optimizacién que consiste en retrasar la
inicializacién o la carga de ciertos aspectos de tu aplicacién hasta que realmente
sean necesarios. Esto puede mejorar significativamente la eficiencia de tu
aplicacién, ya que reduce la cantidad de trabajo que debe realizarse durante la carga
...
React incorpora una funcién llamada React. 1lazy() que te permite renderizar
...
componente de tu aplicacién. Por ejemplo, podrias decidir hacer lazy loading
de ciertas partes de tu aplicacién que no son criticas para la carga inicial de
...
compatibles con la renderizacién en el lado del servidor (SSR). Para las
bibliotecas que admiten SSR, puedes usar una solucién mas tradicional como
...
el rendimiento de tu aplicacién. Al retrasar la carga de ciertos aspectos de tu
aplicacién hasta que sean necesarios, puedes reducir la cantidad de trabajo que se
...
puede causar un renderizado en todos los componentes que estén conectados a ese
store.
...
En este capitulo, hemos profundizado en los conceptos de optimizacién y
rendimiento en React. Hemos explorado cémo la paginacién de listas puede ayudar
a manejar grandes conjuntos de datos de manera eficiente, cémo la memoizacién
puede prevenir renderizados innecesarios, y cémo la divisién de cédigo y el
lazy loading pueden mejorar el tiempo de carga de nuestra aplicacién. Ademas,
discutimos varias mejores practicas de rendimiento en React, como el uso eficiente
del Contexto y Redux.

Es importante recordar que la optimizacién y el rendimiento son aspectos cruciales
del desarrollo de aplicaciones web. Asegurarse de que tu aplicacién se ejecute de
manera eficiente puede llevar a una mejor experiencia del usuario, y puede marcar
la diferencia en la percepcién del usuario sobre la calidad de tu aplicacién.

En el préximo capitulo, cambiaremos nuestra atencién al testing de frontend. El
testing es necesario para el desarrollo de software, que garantiza que tu cédigo
funcione como se espera y ayuda a prevenir bugs antes de que lleguen a produccién.
Capitulo 9: Pruebas y calidad del codigo
Introducci6n al Testing

El desarrollo de software es un proceso complejo y multifacético. No solo implica
escribir cédigo que funcione, sino también asegurarse de que ese cédigo funcione
correctamente en todas las situaciones posibles. Aqui es donde entra en juego el
testing o las pruebas de software.

Las pruebas de software (tests) son una parte integral del ciclo de vida del desarrollo
de software. Nos permiten verificar que nuestro cédigo hace lo que se supone
que debe hacer y nos ayudan a detectar y corregir errores antes de que lleguen a
produccién. En otras palabras, las pruebas nos ayudan a garantizar la calidad de
nuestro software.
...
+ Pruebas de integracién (Integration Tests): Estas pruebas se centran en c6mo
...
- Pruebas de aceptacién (Acceptance Tests): Estas pruebas se realizan desde la
perspectiva del usuario final y se aseguran de que el sistema cumpla con los
requisitos especificados.

En este capitulo, nos centraremos en las pruebas unitarias y de integracién
utilizando Jest y Vitest, las pruebas de componentes con React Testing Library, y
cémo mantener nuestro cédigo limpio y de calidad con Eslint y Prettier. También
cubriremos cémo realizar un anilisis de cobertura de pruebas (Code Coverage)
para asegurarnos de que todas las partes de nuestro cédigo estén adecuadamente
...
Jest es una popular biblioteca de pruebas de JavaScript creada por Facebook.
Es conocida por su simplicidad y facilidad de uso, lo que la hace ideal para
principiantes. Jest es capaz de ejecutar pruebas unitarias y de integracién, y viene
con una serie de caracteristicas utiles como la simulacién de funciones (mocking), la
generacién de informes de cobertura de pruebas, y la capacidad de ejecutar pruebas
...
Una vez instalado, puedes crear un archivo de prueba con la extensién . test. js.
...
Eneste ejemplo, test es una funcién global proporcionada por Jest que define una
prueba. La funcién expect se utiliza para hacer afirmaciones sobre lo que el cédigo
deberia hacer, y .toBe() es un “matcher” que comprueba si el valor esperado
coincide con el valor real.

Vitest

Vitest es una herramienta de pruebas para Vite, el entorno de desarrollo y
compilador para JavaScript. Aunque este libro se centra en React, es importante
...
que la transicién entre las dos herramientas sea bastante sencilla.

Vitest es una excelente opcién para las pruebas, especialmente si estas utilizando
Vite en tu proyecto. Vite es un entorno de desarrollo y compilador moderno y
rapido para JavaScript, y es recomendado por el equipo de React para construir
aplicaciones de una sola pagina (SPA). Vitest esta optimizado para Vite y requiere
practicamente cero configuracién, lo que facilita su uso.

Veamos algunos ejemplos mas complejos de pruebas con Vitest.
Ejemplo 1: Prueba de una funcién asincrona

Supongamos que tienes una funcién asincrona que recupera datos de una API.

Podrias querer probar que esta funcién devuelve los datos esperados.
...
usar await dentro de ella. Esto es necesario porque fetchData es una funcién
asincrona que devuelve una promesa.

Ejemplo 2: Prueba de un componente React.

Vitest también puede ser utilizado para probar componentes React con ayuda de
...
funcién handleC1ick fue llamada cuando se hizo clic en el botén.
...
React. Su filosofia se basa en probar el software de la misma manera que el usuario
...
En este ejemplo, usamos la funcién render para renderizar nuestro componente
...
Ejemplo 2: Prueba de interaccién del usuario
...
En este ejemplo, usamos fireEvent.click para simular un clic en el botén, y
luego verificamos que el texto del botén ha cambiado como se esperaba.
...
software. No solo hace que el cddigo sea mas facil de leer y entender, sino que
también puede ayudar a prevenir errores. Aqui es donde entran en juego ESlint y
Prettier.

ESlint

ESlint es una herramienta de linting para JavaScript. El “linting” es el proceso
...
necesidades. También es extensible, lo que significa que puedes escribir tus propias
...
Esta configuracién habilita las reglas recomendadas para JavaScript y React, y
desactiva la reglareact/prop-types.

Prettier

Prettier es una herramienta de formateo de cédigo para JavaScript. A diferencia
...
Esta configuracién especifica que se deben usar punto y coma al final de las
...
Integraci6n de Eslint y Prettier
ESlint y Prettier pueden trabajar juntos para proporcionar una experiencia de

desarrollo mas fluida. Puedes configurar ESlint para que utilice Prettier como
...
Con esta configuracién, ESlint utilizara Prettier para formatear tu cdédigo y
...
como puede ayudarte a asegurarte de que todas las partes de tu cédigo estén
...
para probar si el flujo de una aplicacién se esta ejecutando como se espera desde el
inicio hasta el final.

El propésito de realizar pruebas de extremo a extremo es identificar problemas del
sistema y garantizar que la aplicacién funciona correctamente en su conjunto. En
...
configuracién, escritura, ejecucién y depuracién de pruebas para aplicaciones web
sea bastante sencilla. Cypress también toma instantaneas o capturas a medida que

se ejecutan tus pruebas. Puedes “viajar en el tiempo” para ver cémo se veia tu

aplicacién en cualquier punto de la prueba.
...
Playwright indica que es una expresién regular, lo que significa que estamos
...
implementar pruebas unitarias y de integracién con Jest y Vitest, como probar
componentes de React con React Testing Library, y como mantener nuestro cédigo
limpio y de alta calidad con Eslint y Prettier. También hemos explorado cé6mo
realizar un analisis de cobertura de pruebas para asegurarnos de que todas
las partes de nuestro cédigo estén adecuadamente probadas. Finalmente, hemos
introducido las pruebas de extremo a extremo con Cypress y Playwright, dos
herramientas que nos permiten asegurarnos de que nuestra aplicacién funciona
...
En el préximo capitulo, nos adentraremos en el despliegue a produccién.

Aprenderemos cémo llevar nuestras aplicaciones de React desde nuestro entorno
de desarrollo local hasta un entorno de produccién, donde los usuarios podran
...
+ Herman, D. (2012). Effective JavaScript: 68 Specific Ways to Harness the Power of
JavaScript. Addison-Wesley Professional.
...
«Jest. (2023). Jest Documentation. Facebook. https://jestjs.io
...
Capitulo 10: Despliegue y
Entornos de Produccion
Introducci6n

En este capitulo final, exploraremos cémo llevar nuestras aplicaciones React desde
nuestro entorno de desarrollo local hasta un entorno de produccién optimizado,

listo para ser consumido por los usuarios finales.

Abordaremos la creacién de un entorno de produccién, el despliegue en
plataformas populares como Netlify y Vercel, y finalmente, discutiremos sobre SEO

y rendimiento en aplicaciones de React.
Creacién de un entorno de produccién optimizado

Cuando desarrollamos una aplicacién, generalmente trabajamos en un entorno de
desarrollo. Este entorno esta configurado para facilitar la depuracién y las pruebas,
pero no esta optimizado para el rendimiento. Al estar listos para desplegar nuestra
aplicacién, es esencial crear un entorno de produccién optimizado para la velocidad
yeficiencia.

En React (utilizando Vite), logramos esto con el comandonpm run build. Este
comando genera una version de produccién de tu aplicacion en el directorio build.
...
los archivos JavaScript y CSS necesarios. Estos archivos estan listos para ser

desplegados en un servidor de produccién.

Es crucial recordar que la version de produccién de tu aplicacién puede
comportarse de manera ligeramente diferente a la version de desarrollo.

Diferencias entre el entorno de producci6n y el de desarrollo

En el entorno de desarrollo, priorizamos la facilidad de depuracién y la velocidad
de desarrollo. Por ejemplo, en React, el entorno de desarrollo incluye mensajes
de advertencia detallados para ayudarte a entender y corregir problemas en tu
cédigo. También se incluyen herramientas como “hot reloading”, que actualiza
automaticamente tu aplicacién en el navegador a medida que modificas el cédigo,
acelerando el ciclo de desarrollo.

En cambio, en el entorno de produccién, la prioridad es la eficiencia y el
rendimiento. Los mensajes de advertencia detallados y el “hot reloading” son utiles
durante el desarrollo, pero pueden ralentizar tu aplicacién en produccién y exponer
detalles de implementacién que preferirias mantener privados. Por lo tanto, estas

caracteristicas se eliminan en la versién de produccién.
Minificacién y su importancia

La minificacién es el proceso de eliminar todos los datos innecesarios de tu cédigo
sin afectar su funcionalidad. Esto incluye espacios en blanco, comentarios, saltos
de linea, y a veces, incluso los nombres de las variables se acortan. La minificacién
hace que tu cédigo sea mas dificil de leer para las personas, pero no afecta su
interpretacion por parte de la maquina.

La principal ventaja de la minificacién es que reduce el tamajio de tus archivos
JavaScript y CSS, lo que a su vez disminuye el tiempo de carga de tu pagina.
Esto puede tener un impacto significativo en el rendimiento de tu aplicacién,
especialmente para los usuarios con conexiones a internet mas lentas.

Ademis, la minificacién puede ofrecer beneficios de seguridad, ya que hace que tu
cédigo sea mas dificil de entender para alguien que intenta leerlo.
Importancia de probar en el entorno de produccién

Como mencioné anteriormente, debido a estas diferencias entre los entornos de
desarrollo y produccién, es posible que tu aplicacién se comporte de manera
diferente en produccién que en desarrollo. Por lo tanto, siempre es una buena
practica probar tu aplicacién en un entorno que se asemeje lo mAs posible al de

produccién antes de desplegarla. Esto te ayudard a detectar y corregir cualquier
problema antes de que tus usuarios lo hagan.

En el siguiente apartado, hablaremos sobre cémo desplegar nuestra aplicacién en
...
Estas plataformas se encargan de la infraestructura y la gestién del servidor,
permitiéndonos centrarnos en el desarrollo de nuestras aplicaciones. Ademas,
ofrecen caracteristicas como la integracién continua/despliegue continuo (CI/CD),
...
Ahora, vamos a ver cémo desplegar una aplicacién React en cada una de estas
...
aplicaciones de JavaScript, incluyendo React.

Acontinuacién, te guiaré a través de los pasos para desplegar una aplicacién React
en Vercel.

Paso 1: Preparar tu aplicacion React

Antes de desplegar tu aplicacién, debes asegurarte de que esta lista para la
produccién. Esto significa que debes ejecutar elcomandonpm run buildentu
proyecto React, que creard una versién optimizada de tu aplicacién en el directorio
build.
...
Vercel utiliza Git para obtener el cddigo de tu aplicacién. Por lo tanto, necesitas
tener tu aplicacién en un repositorio Git. Si atin no has hecho esto, aqui te muestro
...
8 git commit -m "Mi primera aplicacién React"
...
Una vez que hayas iniciado sesién en Vercel, puedes desplegar tu aplicacién
siguiendo estos pasos:

1. Haz clic en “New Project”.

2. Selecciona tu repositorio de Git. Si has iniciado sesién con tu cuenta de GitHub,
GitLab o Bitbucket, deberias ver tus repositorios en la lista. Si no, puedes
afiadirlos manualmente.

3.En la configuracién de despliegue, asegurate de que el comando de
construccién seanpm run buildyel directorio de salida sea build/. Vercel
deberia detectar estos automaticamente si estas desplegando una aplicacién
React.

4. Haz clic en “Deploy”. Vercel construira y desplegara tu aplicacién.

Una vez que tu aplicacién esté desplegada, recibiras una URL unica donde
podras acceder a ella. Vercel también continuard desplegando automaticamente tu
aplicacién cada vez que hagas un push a tu repositorio de Git.

Ahora tienes tu aplicacién React desplegada en Vercel.
...
que Vercel. Es una excelente opcién para desplegar aplicaciones de JavaScript,
incluyendo también React.

Acontinuacién, te guiaré a través de los pasos para desplegar una aplicacién React
en Netlify.

Paso 1: Preparar tu aplicacién React

Al igual que con Vercel, antes de desplegar tu aplicacién, debes asegurarte de que
esta lista para produccion. Esto significa que debes ejecutar el comandonpm run
build en tu proyecto React, que creara una versién optimizada de tu aplicacién en
el directorio build.

Paso 2: Crear un repositorio Git

Netlify también utiliza Git para obtener el cédigo de tu aplicacién. Por lo tanto,
necesitas tener tu aplicacién en un repositorio Git. Los pasos serfan similares al
apartado con Vercel.
...
Paso 4: Desplegar tu aplicacién en Netlify

Una vez que hayas iniciado sesién, puedes desplegar tu aplicacién siguiendo estos
pasos:

1. Haz clic en el botén “New site from Git”.

2. Selecciona tu proveedor de Git (GitHub, GitLab, Bitbucket) y autoriza a Netlify
para acceder a tus repositorios.

3. Selecciona el repositorio que contiene tu aplicacién React.

4.En la configuracién de despliegue, asegurate de que el comando de
construccién seanpm run buildyel directorio de publicacién sea build/.

5. Haz clic en “Deploy site”. Netlify construira y desplegara tu aplicacién.
Una vez que tu aplicacién esté desplegada, recibiras una URL tnica donde podras
acceder a ella. Netlify también continuard desplegando automaticamente tu

aplicacién cada vez que hagas un push a tu repositorio.

Y eso es todo. Ahora tienes tu aplicacién React desplegada en Netlify.
...
desplegar una aplicacién React en Firebase Hosting.

Paso 1: Preparar tu aplicacién React

Antes de desplegar tu aplicacién, debes asegurarte de que esta lista para la
produccién. Esto significa que debes ejecutar elcomandonpm run buildentu
proyecto React, que crear una versién optimizada de tu aplicacién en el directorio
...
desplegar tu aplicacién en Firebase Hosting. Puedes instalar Firebase CLI con npm:

1 $ npm install -g firebase-tools

Paso 4: Iniciar sesion en Firebase

Una vez que hayas instalado Firebase CLI, puedes iniciar sesién en Firebase con el
siguiente comando:

1 $ firebase login
Esto abrird una nueva ventana en tu navegador web para que inicies sesién en tu
cuenta de Google.
...
build/ es el directorio donde React crea la version de produccién de tu aplicacién.

También puedes ahorrarte el paso del asistente e indicar con un comando que solo
quieres hacer uso del servicio de hosting que provee Firebase:

1 $ firebase init hosting

Al final de la inicializacién, Firebase crea automaticamente y afiade dos archivos a
la raiz del directorio de tu aplicacién local:

Un archivo de configuracién f irebase . json que enumera la configuracién de tu
proyecto. Y un archivo . firebaserc que almacena los alias de tu proyecto.

Paso 6: Desplegar tu aplicacién

Finalmente, puedes desplegar tu aplicacién en Firebase Hosting con el siguiente
comando:

1 $ firebase deploy

2 # también puedes especificar sélo el servicio de hosting al
despl\
...
Este comando subira tu aplicacién al servicio de alojamiento. Una vez que tu
...
cruciales para cualquier aplicacién web cuya propuesta de valor es el contenido, y
las aplicaciones React no son una excepcién. En este apartado, vamos a explorar
cémo optimizar tu aplicacién React para los motores de busqueda y mejorar su
rendimiento.

SEO en React

Las aplicaciones de React son aplicaciones de una sola pagina (SPA), lo que significa
que toda la aplicacién se carga en una tinica pagina o documento HTML y luego se
...
Aqui hay algunas estrategias que puedes usar para mejorar el SEO de tu aplicacién
React:

Uso de la etiqueta de titulo y descripcion
El titulo y la descripcién de tu pagina son dos de los factores mas importantes para

el SEO. Puedes establecer el titulo y la descripcién de tu pagina en React utilizando
...
<div>
6 <Helmet>
? <title>Mi Aplicacién React</title>
8 <meta name="description" content="Esta es mi aplicacién
...
puedes usar para mejorar el SEO de tu aplicacién React. Con SSR, tu aplicacién se
...
facilitan mucho poder tener una aplicacién SSR y mucho mas.
Rendimiento en React

React es conocido por su alto rendimiento, pero hay varias técnicas que puedes usar

para hacer que tu aplicacién React sea atin mas rapida.

Uso de React.memo

Como vimos en capitulos anteriores, React.memo es una funcién de orden
...
También lo vimos en anteriores capitulos, React. lazy y Suspense son dos
...
rendimiento de tu aplicacién React.

Hemos Ilegado al final de nuestro camino explorando el despliegue y el entorno de
produccién en React.js. Hemos aprendido cémo preparar nuestra aplicacién para la
produccién, cémo desplegarla en varias plataformas populares y cémo optimizarla
para el SEO y el rendimiento.

Espero que este capitulo te haya proporcionado una sdlida comprensién de cémo
llevar tu aplicacién React desde tu entorno de desarrollo local hasta un entorno
de produccién listo para ser consumido por los usuarios finales. Recuerda, el
despliegue es solo el comienzo. Una vez que tu aplicacién esté en produccién,
...
aprender sobre React y el desarrollo web en general. Te animo a que sigas
...
« Rauschmayer, A. (2018). JavaScript for impatient programmers. Independently
published.

+ Facebook Inc. (2023). React - A JavaScript library for building user interfaces.
https://react.dev/
...
Experimentamos la flexibilidad y potencia de la gestién del estado global con
...
En los capitulos finales, nos adentramos en el consumo de APIs y la comunicacién
...
discutimos estrategias para mejorar el SEO y el rendimiento en produccién.
...
descubrir. Por ejemplo, puedes aventurarte en el desarrollo de aplicaciones méviles
con React Native o profundizar en técnicas de desarrollo con Server-Side Rendering
(SSR) usando Next.js. También puedes explorar bibliotecas y herramientas como
...
sobre las tltimas tendencias y novedades en React y la comunidad de desarrollo
web.

Estoy convencido de que estas preparado para enfrentar los desafios del desarrollo
front-end en el mundo actual. Espero que este libro te haya servido como guia e
inspiracién para seguir explorando y aprendiendo.

Si es asi, me encantaria conocer tu opinién. Puedes compartir tus impresiones
